 ...
 */
async function handleMessage(message, ext) {
    ...
    if (ignore.channelIds?.includes(message.channelId))
        return;
    if (ignore.userIds?.includes(userId))
        return;
    const memberRoles = message.member?.roles.cache.map(r => r.id) ?? [];
    if (ignore.roleIds?.some(r => memberRoles.includes(r)))
        return;
    // ── Pre-fetch record ──────────────────────────────────────────────────────
    ...
    for (const m of cfg.multipliers ?? []) {
        ...
        if (m.channelId && m.channelId === message.channelId)
            multiplier *= m.multiplier;
    }
    ...
    const newLevelData = (0, XpFormula_1.levelFromXp)(newTotalXp, cfg);
